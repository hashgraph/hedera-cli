import type { APIResponse, TokenResponse, AccountResponse, TokenBalance, DisplayBalanceOptions, DisplayTokenOptions, DisplayOptions } from "../../types";
...
function display(displayFunctionName: string, response: APIResponse, options: DisplayOptions): void {
  displayFunctions[displayFunctionName](response, options);
}

// -- display token functions -- //
function displayTokenKeys(response: APIResponse, options: DisplayTokenOptions): void {
  // TODO: Handle options!
...
function displayBalance(response: APIResponse, options: DisplayBalanceOptions): void {
...
function displayTokenBalance(accountId: string, tokens: TokenBalance[], tokenId: string): void {
  const tokenBalance = tokens.find((token: TokenBalance) => token.token_id === tokenId);
  if (tokenBalance) {
...
function displayAllBalances(accountId: string, hbars: number, tokens: TokenBalance[]): void {
