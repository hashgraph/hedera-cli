...
class Http2Transport {
    constructor(session, subchannelAddress, options) {
        ...
        this.channelzRef = channelz_1.registerChannelzSocket(this.subchannelAddressString, () => this.getChannelzInfo(), this.channelzEnabled);
        session.once('close', () => {
    ...
    createCall(metadata, host, method, listener, subchannelCallStatsTracker) {
        ...
        call = new subchannel_call_1.Http2SubchannelCall(http2Stream, eventTracker, listener, this.subchannelAddressString, call_number_1.getNextCallNumber());
...
class Http2SubchannelConnector {
    ...
    createSession(address, credentials, options, proxyConnectionResult) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (proxyConnectionResult.realTarget) {
                remoteName = uri_parser_1.uriToString(proxyConnectionResult.realTarget);
                this.trace('creating HTTP/2 session through proxy to ' + uri_parser_1.uriToString(proxyConnectionResult.realTarget));
            }
            else {
                remoteName = null;
                this.trace('creating HTTP/2 session to ' + subchannel_address_1.subchannelAddressToString(address));
            }
    ...
    connect(address, credentials, options) {
        ...
        if ('secureContext' in connectionOptions) {
            ...
            if (options['grpc.ssl_target_name_override']) {
                ...
                if ('grpc.http_connect_target' in options) {
                     ...
                     * anyway */
                    const targetPath = resolver_1.getDefaultAuthority((_a = uri_parser_1.parseUri(options['grpc.http_connect_target'])) !== null && _a !== void 0 ? _a : {
                        path: 'localhost',
                    });
        ...
        return http_proxy_1.getProxiedConnection(address, options, connectionOptions).then(result => this.createSession(address, credentials, options, result));
    }
    shutdown() {
        var _a;
